...
const STATUS_CFG: Record<string, { label: string; pill: string }> = {
  PENDING:  { label: "Pending",      pill: "bg-orange-50 text-orange-500 border-orange-100" },
  REVIEW:   { label: "Under Review", pill: "bg-blue-50   text-blue-500   border-blue-100"   },
  APPROVED: { label: "Accepted",     pill: "bg-green-50  text-green-600  border-green-100"  },
  REJECTED: { label: "Rejected",     pill: "bg-red-50    text-red-500    border-red-100"    },
};
...
function UpdateCard({ item }: { item: StatusHistoryItem }) {
  ...
  const title = isApproved ? "Congratulations! Offer Accepted"
    : isRejected            ? "Application Rejected"
    : isReview              ? "Application Under Review"
    :                         "Application Submitted Successfully!";

  const desc = isApproved
    ? "Your acceptance has been confirmed. Welcome to the program!"
    : isRejected
...
function PayBadge({ status, appStatus }: { status?: string; appStatus: string }) {
  ...
  if (s === "completed" || s === "paid")
    return <span className="text-xs px-2.5 py-0.5 rounded-full bg-green-50 text-green-600 border border-green-100 font-semibold">Paid</span>;
  if (appStatus === "REJECTED")
    return <span className="text-xs px-2.5 py-0.5 rounded-full bg-gray-50 text-gray-500 border border-gray-200 font-semibold">Canceled</span>;
  return <span className="text-xs px-2.5 py-0.5 rounded-full bg-orange-50 text-orange-500 border border-orange-100 font-semibold">Pending</span>;
}
...
export default function ApplicationDetailPage() {
  ...
  const handleFlightUpload = async () => {
    ...
    try {
      ...
      showToast.success("Flight ticket uploaded!"); fetchAll();
    } catch (e: any) { showToast.error(e.message || "Upload failed"); }
  ...
  const handleInvoiceUpload = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error("Upload failed");
      setInvoiceFile(null); showToast.success("Invoice uploaded!"); fetchAll();
    } catch (e: any) { showToast.error(e.message || "Upload failed"); }
        ...
        <div className="flex items-center justify-center h-[calc(100vh-64px)]">
          {loading
            ...
            : <div className="text-center">
                <p className="text-[#718096] mb-4">Application not found.</p>
                <Button asChild><Link href="/profile">Back to Dashboard</Link></Button>
              </div>
          }
        </div>
      </div>
      ...
      <main className="pt-[64px]">
        <div className="max-w-[900px] mx-auto px-4 md:px-6 py-6">
          ...
          <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-3 mb-5">
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-[#4A5568] mt-0.5">{application.university?.name || "University"}</p>
            ...
            <div className="text-right shrink-0">
              <p className="text-xs text-[#A0AEC0] mb-1">Application ID: {shortId}</p>
          ...
          <div className="grid md:grid-cols-2 gap-5">
            ...
            <div className="space-y-4">
              ...
              <div className="bg-white rounded-2xl border border-gray-100 p-5">
                <h3 className="font-bold text-[#1B2559] mb-3 text-[15px]">Services Required</h3>
                <div className="divide-y divide-gray-50">
                  {services.length > 0 ? (
                    ...
                    <div className="py-2 text-sm text-[#4A5568]">University Admission</div>
                  )}
                </div>
              </div>
              ...
              {/* Services Note */}
              <div className="bg-white rounded-2xl border border-gray-100 p-5">
                <h3 className="font-bold text-[#1B2559] mb-3 text-[15px]">Services Note</h3>
                <p className="text-sm text-[#4A5568]">
                  {application.additionalNotes || "University Admission"}
                </p>
              </div>

              {/* Payment History */}
              <div className="bg-white rounded-2xl border border-gray-100 p-5">
                <h3 className="font-bold text-[#1B2559] mb-4 text-[15px]">Payment History</h3>
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-100">
                      <th className="text-left pb-2 text-xs font-medium text-[#A0AEC0]">Date</th>
                      <th className="text-left pb-2 text-xs font-medium text-[#A0AEC0]">Description</th>
                      <th className="text-left pb-2 text-xs font-medium text-[#A0AEC0]">Amount</th>
                      <th className="text-left pb-2 text-xs font-medium text-[#A0AEC0]">Status</th>
                    </tr>
                  </thead>
                  <tbody>
                    {application.applicationFee ? (
                      <tr className="border-b border-gray-50">
                        ...
                        <td className="py-3 text-[#4A5568]">Application Fee</td>
                    ...
                    {application.additionalFee && Number(application.additionalFee) > 0 ? (
                      <tr className="border-b border-gray-50">
                        ...
                        <td className="py-3 text-[#4A5568]">Deposit Fee</td>
                    ...
                    {!application.applicationFee && !application.additionalFee && (
                      <tr>
                        <td colSpan={4} className="py-4 text-center text-[#A0AEC0] text-xs">No payment records yet</td>
                      </tr>
                    )}
                  </tbody>
                </table>
                ...
                <div className="mt-4 border border-gray-100 rounded-xl overflow-hidden bg-gray-50/60">
                  <div className="flex justify-between px-4 py-2.5 text-sm border-b border-gray-100">
                    <span className="text-[#718096]">Total Paid:</span>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-white rounded-2xl border border-gray-100 p-5">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="font-bold text-[#1B2559] text-[15px]">Submitted Documents</h3>
                ...
                {documents.length === 0 ? (
                  <p className="text-sm text-[#A0AEC0] text-center py-6">No documents uploaded yet</p>
                ) : (
          ...
          <div className="mt-5 bg-white rounded-2xl border border-gray-100 p-5">
            <h3 className="font-bold text-[#1B2559] mb-4 text-[15px]">Application Advisor</h3>
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-[#A0AEC0]">{application.university?.name || "Univolta"}</p>
