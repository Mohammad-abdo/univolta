...
async function fetchUniversity(slug: string, lang: string) {
  try {
...
async function fetchOtherUniversities(country: string, excludeSlug: string, lang: string) {
  try {
    const response = await fetch(
...
export async function generateMetadata({
  ...
  const alternates = publicAlternates(locale, `/universities/${params.slug}`);
  const university = await fetchUniversity(params.slug, locale);
  if (!university) return alternates;

  const metaTitleRaw = (university as any)?.metaTitle;
  const metaTitle =
    typeof metaTitleRaw === "string" && metaTitleRaw.trim()
      ? metaTitleRaw.trim()
      : "";
  const title = metaTitle || university.name || "University";
  const description =
    (university.description || university.about || university.summary || "")
      .toString()
      .trim() || tServer("discoverWorldClassAcademics", locale);
...
export default async function UniversityDetailPage({
  ...
  const university = await fetchUniversity(slug, lang);
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      },
      {
        ...
        item: absoluteLocaleUrl(lang, `/universities/${university.slug}`),
      },
    ],
  };
        ...
        <div className="max-w-[1440px] mx-auto px-4 md:px-5">
          ...
          <div className="relative h-[260px] md:h-[420px] rounded-[20px] md:rounded-[28px] overflow-hidden mb-6 md:mb-10 animate-hero-reveal">
            <Image
              ...
              unoptimized
            />
            <div className="absolute inset-0 bg-gradient-to-r from-[#121c67]/55 via-[#121c67]/30 to-transparent" />
            <div className="absolute inset-0 bg-gradient-to-t from-black/32 via-transparent to-transparent" />

            {/* Admission status badge: stacked below ranking on mobile, top-left on desktop */}
            <div className="absolute top-14 left-4 md:top-4 md:left-4 z-20">
              {(() => {
            ...
            {/* Ranking badge: top on mobile, top-right on desktop */}
            ...
            >
              ...
              <div className={`shrink-0 w-full md:w-auto ${lang === "ar" ? "text-left md:text-left" : "text-right md:text-right"}`}>
                <Button
                  ...
                  asChild
                >
                  <Link href={locHref(`/universities/${slug}/programs`)} className="flex items-center gap-2">
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-2xl border border-gray-100 p-5 md:p-6 flex flex-col gap-5 lg:sticky lg:top-[120px] shadow-sm animate-fade-up">
                ...
                <div className="flex gap-2 md:gap-5">
                  <Link
                    href={locHref(`/universities/${slug}/programs?degree=bachelor`)}
                  ...
                  <Link
                    href={locHref(`/universities/${slug}/programs?degree=master`)}
                      ...
                      >
                        ...
                        <div className="flex flex-col gap-2">
                          ...
                          {programs.length > 5 && (
                            <Link
                              href={locHref(
                                `/universities/${slug}/programs?department=${encodeURIComponent(
                                  deptKey
                                )}`
                              )}
                    ...
                    <Card key={uni.id} className="group overflow-hidden border border-gray-100 shadow-md hover:shadow-2xl transition-all duration-400 hover:-translate-y-2 bg-white h-full flex flex-col">
                      ...
                      <CardFooter className="pt-3 pb-5 px-5">
                        <Button className="w-full bg-[#5260ce] hover:bg-[#4350b0] text-white font-montserrat-semibold text-sm h-11 rounded-xl transition-all duration-300 hover:shadow-[0_8px_24px_rgba(82,96,206,0.35)] group/btn" asChild>
                          <Link href={locHref(`/universities/${uni.slug}`)} className="flex items-center justify-center gap-2">
                            {t("viewDetails")}
