...
export default function ProfilePage() {
  ...
  const loginRedirect = `${withLocalePath(locale, "/login")}?redirect=${encodeURIComponent(
  ...
  useEffect(() => {
    ...
    if (!token) { router.replace(loginRedirect); return; }
          ...
          <div className="mb-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            ...
            <div className="flex items-center gap-2 shrink-0">
              <Button variant="outline"
                ...
                asChild
              >
                <LocaleLink href="/profile/settings"><Settings className="w-4 h-4" /> {t("settings")}</LocaleLink>
              </Button>
              <Button
                className="bg-[#5260ce] hover:bg-[#4350b0] text-white rounded-xl h-10 px-5 text-sm font-semibold"
                asChild
              >
                <LocaleLink href="/universities">{t("browseUniversities")}</LocaleLink>
              </Button>
            </div>
          </div>
                        ...
                        <tr key={app.id} className="border-b border-gray-50 last:border-0 hover:bg-[#FAFBFF] transition-colors group">
                          ...
                          <td className="px-4 py-4">
                            <Button
                              ...
                              asChild
                            >
                              <LocaleLink href={`/my-applications/${app.id}`} className="flex items-center gap-1.5">
                                <EyeIcon />
                                {t("viewDetails")}
                              </LocaleLink>
                            </Button>
