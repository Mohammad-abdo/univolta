...
import { absoluteLocaleUrl, toAppLocale, withLocalePath } from "@/lib/locale-path";
import { publicAlternates } from "@/lib/seo-metadata";
import type { Metadata } from "next";

async function fetchProgram(slug: string, lang: string) {
  try {
...
export async function generateMetadata({
  ...
  params: { locale: string; slug: string; programSlug: string };
}): Promise<Metadata> {
  const locale = toAppLocale(params.locale);
  const alternates = publicAlternates(
    locale,
    `/universities/${params.slug}/programs/${params.programSlug}`
  );
  const program = await fetchProgram(params.programSlug, locale);
  if (!program) return alternates;

  const titleSuffix = tServer("programs", locale);
  const title = program.name ? `${program.name} | ${titleSuffix}` : titleSuffix;
  const description =
    (program.description || program.overview || program.summary || "")
      .toString()
      .trim() || tServer("programsSectionSubtitle", locale);
...
async function fetchUniversityPrograms(universitySlug: string, lang: string) {
  try {
...
export default async function ProgramDetailPage({
  ...
  const locHref = (path: string) => withLocalePath(lang, path);
  const program = await fetchProgram(programSlug, lang);

  if (!program) notFound();

  const t = (key: string) => tServer(key, lang);
  const universitySlug = slug || program.university?.slug || "";
    ...
    itemListElement: [
      {
        ...
        position: 1,
        name: t("home"),
        item: absoluteLocaleUrl(lang, "/"),
      },
      {
        "@type": "ListItem",
        position: 2,
        name: t("universities"),
        item: absoluteLocaleUrl(lang, "/universities"),
      },
      {
        "@type": "ListItem",
        position: 3,
        name: program.university?.name || t("universities"),
        item: absoluteLocaleUrl(lang, `/universities/${universitySlug}`),
      },
      {
        "@type": "ListItem",
        position: 4,
        name: program.name,
        item: absoluteLocaleUrl(
          lang,
  ...
  const universityPrograms = await fetchUniversityPrograms(universitySlug, lang);
      ...
      <main className="pt-0 md:pt-[110px] pb-16 md:pb-24">
        <div className="max-w-[1440px] mx-auto px-4 md:px-5">
          ...
          <div className="relative h-[280px] md:h-[520px] rounded-[20px] md:rounded-[32px] overflow-hidden mb-8 md:mb-10 animate-hero-reveal shadow-[0_24px_80px_rgba(82,96,206,0.2)]">
            ...
            {/* Layered gradients */}
            <div className="absolute inset-0 bg-gradient-to-r from-[#121c67]/58 via-[#121c67]/32 to-transparent" />
            <div className="absolute inset-0 bg-gradient-to-t from-[#121c67]/48 via-transparent to-transparent" />
            ...
            <div className="absolute top-4 md:top-6 left-5 md:left-8 right-5 md:right-8 flex items-start justify-between">
              ...
              <nav className="hidden md:flex items-center gap-1.5 text-white/60 text-xs font-montserrat-regular animate-fade-up">
                <Link href={locHref("/")} className="hover:text-white/90 transition-colors">{t("breadcrumbHome")}</Link>
                <ChevronRight className="w-3 h-3" />
                <Link href={locHref("/universities")} className="hover:text-white/90 transition-colors">{t("breadcrumbUniversities")}</Link>
                <ChevronRight className="w-3 h-3" />
                <Link href={locHref(`/universities/${slug}`)} className="hover:text-white/90 transition-colors">{program.university?.name}</Link>
                <ChevronRight className="w-3 h-3" />
                <Link href={locHref(`/universities/${slug}/programs`)} className="hover:text-white/90 transition-colors">{t("breadcrumbPrograms")}</Link>
                <ChevronRight className="w-3 h-3" />
              ...
              <div className="bg-white rounded-2xl border border-gray-100 p-5 md:p-6 shadow-sm lg:sticky lg:top-[130px]">
                      ...
                      <div className="flex flex-col gap-0.5">
                        {programs.map((p: any) => {
                          ...
                          return (
                            <Link
                              key={p.id}
                              href={locHref(`/universities/${slug}/programs/${p.slug}`)}
                ...
                <div className="mt-4 pt-4 border-t border-gray-100">
                  <Button className="w-full bg-[#5260ce] hover:bg-[#4350b0] text-white font-montserrat-semibold h-11 rounded-xl shadow-[0_4px_16px_rgba(82,96,206,0.3)] hover:shadow-[0_6px_20px_rgba(82,96,206,0.4)] transition-all group" asChild>
                    <Link href={locHref(`/universities/${slug}/programs`)} className="flex items-center justify-center gap-2">
                      {t("viewAllPrograms")}
                  ...
                  return (
                    <Card key={similar.id} className="group overflow-hidden border border-gray-100 shadow-md hover:shadow-2xl transition-all duration-400 hover:-translate-y-2 bg-white h-full flex flex-col">
                      ...
                      <CardFooter className="pt-3 pb-5 px-5">
                        ...
                        >
                          <Link
                            href={locHref(`/universities/${similar.university?.slug || slug}/programs/${similar.slug}`)}
